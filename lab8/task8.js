...
function saveButton(){
	var saveUpload = [];
	if (l === 1){
		d = document.querySelector('.saver');
	...
	saveUpload.field1 = document.body.querySelector('table').querySelector('.'+nRow[i].className).querySelector('.field1').value;
		if (saveUpload.field1 == empty) { saveUpload.field1 = "вы не ввели слово!";}
	saveUpload.field2  = document.body.querySelector('table').querySelector('.'+nRow[i].className).querySelector('.field2').value;
		if (saveUpload.field2 == empty) { saveUpload.field2 = "вы не ввели число!";}
	result = result + saveUpload.field1 + " : " + saveUpload.field2;
	if (i!=n) {result = result + " , "};

	
	}
		document.body.insertAdjacentHTML("beforeend",  `<div class="saver">{"${result}"}</div>`);
	}
}
function listener(){
	nRow[0]=document.querySelector('.row0');
	empty = document.body.querySelector('table').querySelector('.row0').querySelector('.field1').value;
	nRow[-1]=null;
